...
const userRole = computed(() => normalizeRole(currentUser.value?.role));
const isAdmin = computed(() => userRole.value === "ADMIN");

const adminMenu = [
  { to: "/admin", label: "管理首页", icon: "总", exact: true },
  { to: "/admin/users", label: "用户管理", icon: "户" },
];

const cadreMenu = [
  { to: "/cadre", label: "工作首页", icon: "总", exact: true },
  { to: "/cadre/announcements", label: "公告管理", icon: "告" },
  { to: "/cadre/affairs", label: "村务管理", icon: "务" },
...
const menu = computed(() => (isAdmin.value ? adminMenu : cadreMenu));
    ...
    <aside class="sidebar">
      <div class="sidebar-brand">
        <RouterLink to="/" class="brand-link">
          ...
          <span class="brand-text">智慧乡村</span>
        </RouterLink>
      </div>
    ...
    <div class="main-area">
      <header class="topbar">
        <div class="topbar-left">
          <span class="section-kicker">{{ isAdmin ? "系统权限" : "村务办理" }}</span>
          <strong class="topbar-title">{{ isAdmin ? "管理员工作台" : "干部工作台" }}</strong>
        </div>
        ...
        <div class="topbar-right">
          ...
          <span class="user-role sv-tag" :class="isAdmin ? 'sv-tag--approved' : 'sv-tag--processing'">
            {{ isAdmin ? "管理员" : "村干部" }}
          </span>
