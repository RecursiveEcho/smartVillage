...
public class MediaBindEventListener {
  ...
  private final Map<String, MediaBinder>
      binderMap; // {"FEATURE"→FeatureBinder, "ANNOUNCEMENT"→AnnouncementBinder, ...}
  ...
  public void onMediaUploaded(MediaBindAfterUploadEvent event) {
    ...
    if (binder == null)
      throw new BusinessException(
          ErrorCode.PARAM_INVALID, "不支持的 bindTarget: " + target); // 不支持的目标，报错
