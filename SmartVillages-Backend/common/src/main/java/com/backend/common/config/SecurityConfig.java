...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)//禁用CSRF保护
            ...
            // 管理员仅负责账号管理
            .requestMatchers("/admin/users/**", "/admin/me", "/media/images/upload").hasAuthority("ROLE_ADMIN")
            // 村干部负责业务处理（公告、留言）
            .requestMatchers("/cadre/announcements/**", "/cadre/interactions/**", "/media/image/upload").hasAuthority("ROLE_CADRE")
