 ...
 *
 * <p>
 * 状态约定：0-待审核 1-已通过(发布) 2-已拒绝 3-已下架。 详情接口先读缓存；缓存命中时用 SQL 对 {@code view_count}
 * 做原子 +1 并同步
 * VO，未命中则查库更新后回写缓存。
 */
@Service
...
public class AnnouncementServiceImpl extends ServiceImpl<AnnouncementMapper, AnnouncementEntity>
  ...
  public void updateAnnouncement(Long id, AnnouncementUpdateDTO dto, HttpServletRequest request) {
    ...
    try {
      ...
      entity.setIsTop(dto.getIsTop());
      updateById(entity);
      invalidateCachesAfterCommit(id);
    } finally {
  ...
  // 在事务提交后清理缓存，避免事务回滚导致缓存不一致
  private void invalidateCachesAfterCommit(Long id) {
    // 注册事务同步器，在事务提交后执行缓存清理操作
