...
public class VillagePopulationServiceImpl
  ...
  private static final String CACHE_KEY_PREFIX = "village_population:detail:";
  ...
  public void createVillagePopulation(VillagePopulationCreateDTO villagePopulationCreateDTO) {
    ...
    redisJsonCacheTool.setObject(
        CacheKeyUtils.detailKey(CACHE_KEY_PREFIX, villagePopulationEntity.getId()),
        villagePopulationEntity);
   ...
   */
  @Override
  public IPage<VillagePopulationSimpleVO> getVillagePopulationList(
      Long current,
      Long size,
      String householdNo,
      String fullName,
    ...
    String prefix =
        CACHE_LIST_PREFIX
            + CacheKeyUtils.listFilterSegment(householdNo, fullName, gender, relationToHead);
    String listKey = redisJsonCacheTool.buildVersionedListPageKey(prefix, ver, current, size);
    VillagePopulationListPageCache cached =
        redisJsonCacheTool.getObject(listKey, VillagePopulationListPageCache.class);
    if (cached != null) {
      List<VillagePopulationSimpleVO> rows =
          cached.getRecords() != null ? cached.getRecords() : Collections.emptyList();
      Page<VillagePopulationSimpleVO> hit =
          new Page<>(cached.getCurrent(), cached.getSize(), cached.getTotal());
      hit.setRecords(rows);
      return hit;
    }
    LambdaQueryWrapper<VillagePopulationEntity> queryWrapper =
        new LambdaQueryWrapper<VillagePopulationEntity>()
            .like(
                StringUtils.hasText(householdNo),
                VillagePopulationEntity::getHouseholdNo,
                householdNo)
            ...
            .eq(
                StringUtils.hasText(relationToHead),
                VillagePopulationEntity::getRelationToHead,
                relationToHead)
  ...
  public VillagePopulationDetailVO getVillagePopulationDetail(Long id) {
    ...
    VillagePopulationDetailVO cached =
        redisJsonCacheTool.getObject(
            cacheKey,
            VillagePopulationDetailVO.class,
            () -> {
