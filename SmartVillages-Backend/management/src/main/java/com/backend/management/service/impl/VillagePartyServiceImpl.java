...
public class VillagePartyServiceImpl extends ServiceImpl<VillagePartyMapper, VillagePartyEntity>
    implements VillagePartyService {

  private static final String CACHE_KEY_PREFIX = "village_party:detail:";
   ...
   */
  @Override
  public IPage<VillagePartySimpleVO> getList(
      Long current, Long size, String orgName, String orgType, String secretaryName) {
    String ver = redisJsonCacheTool.getListCacheVersionOrZero(CACHE_LIST_VER_KEY);
    String prefix =
        CACHE_LIST_PREFIX + CacheKeyUtils.listFilterSegment(orgName, orgType, secretaryName);
    String listKey = redisJsonCacheTool.buildVersionedListPageKey(prefix, ver, current, size);
    VillagePartyListPageCache cached =
        redisJsonCacheTool.getObject(listKey, VillagePartyListPageCache.class);
    if (cached != null) {
      List<VillagePartySimpleVO> rows =
          cached.getRecords() != null ? cached.getRecords() : Collections.emptyList();
      Page<VillagePartySimpleVO> hit =
          new Page<>(cached.getCurrent(), cached.getSize(), cached.getTotal());
      hit.setRecords(rows);
      return hit;
    }
    LambdaQueryWrapper<VillagePartyEntity> queryWrapper =
        new LambdaQueryWrapper<VillagePartyEntity>()
            ...
            .like(
                StringUtils.hasText(secretaryName),
                VillagePartyEntity::getSecretaryName,
                secretaryName)
  ...
  public VillagePartyDetailVO getDetail(Integer id) {
    ...
    VillagePartyDetailVO fromCache =
        redisJsonCacheTool.getObject(
            cacheKey,
            VillagePartyDetailVO.class,
            () -> {
