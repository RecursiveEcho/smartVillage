...
public class VillageAffairServiceImpl extends ServiceImpl<VillageAffairMapper, VillageAffairEntity>
  ...
  */
   ...
   */
  @Override
  public IPage<VillageAffairSimpleVO> getList(
      Long current, Long size, Integer status, String affairType, String title) {
    String ver = redisJsonCacheTool.getListCacheVersionOrZero(CACHE_LIST_VER_KEY);
    String prefix =
        CACHE_LIST_ADMIN_PREFIX + CacheKeyUtils.listFilterSegment(status, affairType, title);
    String listKey = redisJsonCacheTool.buildVersionedListPageKey(prefix, ver, current, size);
    VillageAffairListPageCache cached =
        redisJsonCacheTool.getObject(listKey, VillageAffairListPageCache.class);
    if (cached != null) {
      List<VillageAffairSimpleVO> rows =
          cached.getRecords() != null ? cached.getRecords() : Collections.emptyList();
      Page<VillageAffairSimpleVO> hit =
          new Page<>(cached.getCurrent(), cached.getSize(), cached.getTotal());
      hit.setRecords(rows);
      return hit;
    }
    LambdaQueryWrapper<VillageAffairEntity> queryWrapper =
        new LambdaQueryWrapper<VillageAffairEntity>()
  ...
  public VillageAffairDetailVO getDetail(Integer id) {
    ...
    VillageAffairDetailVO fromCache =
        redisJsonCacheTool.getObject(
            cacheKey,
            VillageAffairDetailVO.class,
            () -> {
  ...
  */
  ...
  public void audit(Integer id, VillageAffairAuditDTO dto) {
    ...
    boolean locked = redisDistributedLock.tryLock(lockKey, lockInstance);
    if (!locked) {
      throw new BusinessException(ErrorCode.SYSTEM_BUSY, "村务事项正在审核，请稍后再试");
    }
    try {
      VillageAffairEntity entity = getById(id);
      if (entity == null) {
  ...
  @Override
  public IPage<VillageAffairSimpleVO> getPublicList(
      Long current, Long size, String affairType, String title) {
    String ver = redisJsonCacheTool.getListCacheVersionOrZero(CACHE_LIST_VER_KEY);
