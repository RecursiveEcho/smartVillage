...
public class VillageHouseLandServiceImpl
  ...
  private static final String CACHE_KEY_PREFIX = "village_house_land:detail:";
  ...
  public Integer createVillageHouseLand(VillageHouseLandCreateDTO villageHouseLandCreateDTO) {
    ...
    redisJsonCacheTool.setObject(
        CacheKeyUtils.detailKey(CACHE_KEY_PREFIX, id), villageHouseLandEntity);
   ...
   */
  @Override
  public IPage<VillageHouseLandSimpleVO> getVillageHouseLandList(
      Long current, Long size, String bizType, String ownerName, String location) {
    String ver = redisJsonCacheTool.getListCacheVersionOrZero(CACHE_LIST_VER_KEY);
    String prefix =
        CACHE_LIST_PREFIX + CacheKeyUtils.listFilterSegment(bizType, ownerName, location);
    String listKey = redisJsonCacheTool.buildVersionedListPageKey(prefix, ver, current, size);
    VillageHouseLandListPageCache cached =
        redisJsonCacheTool.getObject(listKey, VillageHouseLandListPageCache.class);
    if (cached != null) {
      List<VillageHouseLandSimpleVO> rows =
          cached.getRecords() != null ? cached.getRecords() : Collections.emptyList();
      Page<VillageHouseLandSimpleVO> hit =
          new Page<>(cached.getCurrent(), cached.getSize(), cached.getTotal());
      hit.setRecords(rows);
      return hit;
    }
    LambdaQueryWrapper<VillageHouseLandEntity> queryWrapper =
        new LambdaQueryWrapper<VillageHouseLandEntity>()
  ...
  public VillageHouseLandDetailVO getVillageHouseLandDetail(Integer id) {
    ...
    VillageHouseLandDetailVO fromCache =
        redisJsonCacheTool.getObject(
            cacheKey,
            VillageHouseLandDetailVO.class,
            () -> {
