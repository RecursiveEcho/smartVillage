...
public class MediaServiceImpl extends ServiceImpl<MediaMapper, MediaEntity>
  ...
  public UploadVO upload(
    ...
    if (anyBind && !fullBind) {
      throw new BusinessException(
          ErrorCode.PARAM_INVALID, "bindTarget、bindEntityId、bindSlot 需同时填写");
    }
    ...
    try {
      OssUploadTool.UploadResult uploadResult =
          ossUploadTool.uploadFile(
              file.getInputStream(), fileType, file.getOriginalFilename(), file.getContentType());
  ...
  public IPage<PageVO> page(
      Long current, Long size, String fileType, String category, Integer status) {
  ...
  public void auditMedia(Integer id, Integer status, HttpServletRequest request) {
    ...
    if (status == null
        || (!Objects.equals(status, STATUS_APPROVED) && !Objects.equals(status, STATUS_REJECTED))) {
  ...
  public IPage<PageVO> pageAudited(Long current, Long size, String fileType, String category) {
    String ver = redisJsonCacheTool.getListCacheVersionOrZero(CACHE_LIST_VER_KEY);
    String prefix =
        CACHE_LIST_PREFIX + CacheKeyUtils.listFilterSegment(fileType, category, null) + "_audited";
    String listKey = redisJsonCacheTool.buildVersionedListPageKey(prefix, ver, current, size);
    MediaListPageCache cached = redisJsonCacheTool.getObject(listKey, MediaListPageCache.class);
    if (cached != null) {
      List<PageVO> rows =
